; ModuleID = 'lcc'
source_filename = "lcc"

@0 = private unnamed_addr constant [17 x i8] c"15.logic.c PASS\0A\00", align 1
@1 = private unnamed_addr constant [17 x i8] c"15.logic.c FAIL\0A\00", align 1

declare i32 @printf(ptr, ...)

define i32 @main() !dbg !2 {
entry:
  %ui = alloca i32, align 4
    #dbg_declare(ptr %ui, !7, !DIExpression(), !9)
  %si = alloca i32, align 4
    #dbg_declare(ptr %si, !10, !DIExpression(), !11)
  %ub = alloca i32, align 4
    #dbg_declare(ptr %ub, !12, !DIExpression(), !13)
  %ua = alloca i32, align 4
    #dbg_declare(ptr %ua, !14, !DIExpression(), !15)
  %c = alloca i32, align 4
    #dbg_declare(ptr %c, !16, !DIExpression(), !17)
  %b = alloca i32, align 4
    #dbg_declare(ptr %b, !18, !DIExpression(), !19)
  %a = alloca i32, align 4
    #dbg_declare(ptr %a, !20, !DIExpression(), !21)
  %v = alloca i32, align 4
    #dbg_declare(ptr %v, !22, !DIExpression(), !23)
  %f = alloca i1, align 1
    #dbg_declare(ptr %f, !24, !DIExpression(), !26)
  %t = alloca i1, align 1
    #dbg_declare(ptr %t, !27, !DIExpression(), !28)
  %err = alloca i32, align 4
    #dbg_declare(ptr %err, !29, !DIExpression(), !30)
  store i32 0, ptr %err, align 4, !dbg !30
  store i1 true, ptr %t, align 1, !dbg !28
  store i1 false, ptr %f, align 1, !dbg !26
  %0 = load i1, ptr %t, align 1, !dbg !31
  %1 = zext i1 %0 to i32, !dbg !31
  %2 = icmp eq i32 %1, 0, !dbg !31
  br i1 %2, label %then, label %else, !dbg !31

then:                                             ; preds = %entry
  store i32 1, ptr %err, align 4, !dbg !32
  %3 = load i32, ptr %err, align 4, !dbg !32
  br label %if.end, !dbg !32

else:                                             ; preds = %entry
  br label %if.end, !dbg !32

if.end:                                           ; preds = %else, %then
  %4 = load i1, ptr %f, align 1, !dbg !33
  %5 = zext i1 %4 to i32, !dbg !33
  %6 = icmp ne i32 %5, 0, !dbg !33
  br i1 %6, label %then1, label %else2, !dbg !33

then1:                                            ; preds = %if.end
  store i32 1, ptr %err, align 4, !dbg !34
  %7 = load i32, ptr %err, align 4, !dbg !34
  br label %if.end3, !dbg !34

else2:                                            ; preds = %if.end
  br label %if.end3, !dbg !34

if.end3:                                          ; preds = %else2, %then1
  %8 = load i1, ptr %t, align 1, !dbg !35
  %9 = load i1, ptr %f, align 1, !dbg !35
  %10 = select i1 %8, i1 %9, i1 false, !dbg !35
  %11 = zext i1 %10 to i32, !dbg !35
  store i32 %11, ptr %v, align 4, !dbg !35
  %12 = load i32, ptr %v, align 4, !dbg !35
  %13 = load i32, ptr %v, align 4, !dbg !36
  %14 = icmp ne i32 %13, 0, !dbg !36
  br i1 %14, label %then4, label %else5, !dbg !36

then4:                                            ; preds = %if.end3
  store i32 1, ptr %err, align 4, !dbg !37
  %15 = load i32, ptr %err, align 4, !dbg !37
  br label %if.end6, !dbg !37

else5:                                            ; preds = %if.end3
  br label %if.end6, !dbg !37

if.end6:                                          ; preds = %else5, %then4
  %16 = load i1, ptr %t, align 1, !dbg !38
  %17 = load i1, ptr %t, align 1, !dbg !38
  %18 = select i1 %16, i1 %17, i1 false, !dbg !38
  %19 = zext i1 %18 to i32, !dbg !38
  store i32 %19, ptr %v, align 4, !dbg !38
  %20 = load i32, ptr %v, align 4, !dbg !38
  %21 = load i32, ptr %v, align 4, !dbg !39
  %22 = icmp eq i32 %21, 0, !dbg !39
  br i1 %22, label %then7, label %else8, !dbg !39

then7:                                            ; preds = %if.end6
  store i32 1, ptr %err, align 4, !dbg !40
  %23 = load i32, ptr %err, align 4, !dbg !40
  br label %if.end9, !dbg !40

else8:                                            ; preds = %if.end6
  br label %if.end9, !dbg !40

if.end9:                                          ; preds = %else8, %then7
  %24 = load i1, ptr %f, align 1, !dbg !41
  %25 = load i1, ptr %f, align 1, !dbg !41
  %26 = select i1 %24, i1 %25, i1 false, !dbg !41
  %27 = zext i1 %26 to i32, !dbg !41
  store i32 %27, ptr %v, align 4, !dbg !41
  %28 = load i32, ptr %v, align 4, !dbg !41
  %29 = load i32, ptr %v, align 4, !dbg !42
  %30 = icmp ne i32 %29, 0, !dbg !42
  br i1 %30, label %then10, label %else11, !dbg !42

then10:                                           ; preds = %if.end9
  store i32 1, ptr %err, align 4, !dbg !43
  %31 = load i32, ptr %err, align 4, !dbg !43
  br label %if.end12, !dbg !43

else11:                                           ; preds = %if.end9
  br label %if.end12, !dbg !43

if.end12:                                         ; preds = %else11, %then10
  %32 = load i1, ptr %t, align 1, !dbg !44
  %33 = load i1, ptr %f, align 1, !dbg !44
  %34 = select i1 %32, i1 true, i1 %33, !dbg !44
  %35 = zext i1 %34 to i32, !dbg !44
  store i32 %35, ptr %v, align 4, !dbg !44
  %36 = load i32, ptr %v, align 4, !dbg !44
  %37 = load i32, ptr %v, align 4, !dbg !45
  %38 = icmp eq i32 %37, 0, !dbg !45
  br i1 %38, label %then13, label %else14, !dbg !45

then13:                                           ; preds = %if.end12
  store i32 1, ptr %err, align 4, !dbg !46
  %39 = load i32, ptr %err, align 4, !dbg !46
  br label %if.end15, !dbg !46

else14:                                           ; preds = %if.end12
  br label %if.end15, !dbg !46

if.end15:                                         ; preds = %else14, %then13
  %40 = load i1, ptr %t, align 1, !dbg !47
  %41 = load i1, ptr %t, align 1, !dbg !47
  %42 = select i1 %40, i1 true, i1 %41, !dbg !47
  %43 = zext i1 %42 to i32, !dbg !47
  store i32 %43, ptr %v, align 4, !dbg !47
  %44 = load i32, ptr %v, align 4, !dbg !47
  %45 = load i32, ptr %v, align 4, !dbg !48
  %46 = icmp eq i32 %45, 0, !dbg !48
  br i1 %46, label %then16, label %else17, !dbg !48

then16:                                           ; preds = %if.end15
  store i32 1, ptr %err, align 4, !dbg !49
  %47 = load i32, ptr %err, align 4, !dbg !49
  br label %if.end18, !dbg !49

else17:                                           ; preds = %if.end15
  br label %if.end18, !dbg !49

if.end18:                                         ; preds = %else17, %then16
  %48 = load i1, ptr %f, align 1, !dbg !50
  %49 = load i1, ptr %f, align 1, !dbg !50
  %50 = select i1 %48, i1 true, i1 %49, !dbg !50
  %51 = zext i1 %50 to i32, !dbg !50
  store i32 %51, ptr %v, align 4, !dbg !50
  %52 = load i32, ptr %v, align 4, !dbg !50
  %53 = load i32, ptr %v, align 4, !dbg !51
  %54 = icmp ne i32 %53, 0, !dbg !51
  br i1 %54, label %then19, label %else20, !dbg !51

then19:                                           ; preds = %if.end18
  store i32 1, ptr %err, align 4, !dbg !52
  %55 = load i32, ptr %err, align 4, !dbg !52
  br label %if.end21, !dbg !52

else20:                                           ; preds = %if.end18
  br label %if.end21, !dbg !52

if.end21:                                         ; preds = %else20, %then19
  %56 = load i1, ptr %t, align 1, !dbg !53
  %57 = icmp eq i1 %56, false, !dbg !53
  %58 = zext i1 %57 to i32, !dbg !53
  store i32 %58, ptr %v, align 4, !dbg !53
  %59 = load i32, ptr %v, align 4, !dbg !53
  %60 = load i32, ptr %v, align 4, !dbg !54
  %61 = icmp ne i32 %60, 0, !dbg !54
  br i1 %61, label %then22, label %else23, !dbg !54

then22:                                           ; preds = %if.end21
  store i32 1, ptr %err, align 4, !dbg !55
  %62 = load i32, ptr %err, align 4, !dbg !55
  br label %if.end24, !dbg !55

else23:                                           ; preds = %if.end21
  br label %if.end24, !dbg !55

if.end24:                                         ; preds = %else23, %then22
  %63 = load i1, ptr %f, align 1, !dbg !56
  %64 = icmp eq i1 %63, false, !dbg !56
  %65 = zext i1 %64 to i32, !dbg !56
  store i32 %65, ptr %v, align 4, !dbg !56
  %66 = load i32, ptr %v, align 4, !dbg !56
  %67 = load i32, ptr %v, align 4, !dbg !57
  %68 = icmp eq i32 %67, 0, !dbg !57
  br i1 %68, label %then25, label %else26, !dbg !57

then25:                                           ; preds = %if.end24
  store i32 1, ptr %err, align 4, !dbg !58
  %69 = load i32, ptr %err, align 4, !dbg !58
  br label %if.end27, !dbg !58

else26:                                           ; preds = %if.end24
  br label %if.end27, !dbg !58

if.end27:                                         ; preds = %else26, %then25
  store i32 100, ptr %a, align 4, !dbg !21
  store i32 100, ptr %b, align 4, !dbg !19
  store i32 128, ptr %c, align 4, !dbg !17
  %70 = load i32, ptr %a, align 4, !dbg !59
  %71 = load i32, ptr %b, align 4, !dbg !59
  %72 = icmp eq i32 %70, %71, !dbg !59
  %73 = zext i1 %72 to i32, !dbg !59
  store i32 %73, ptr %v, align 4, !dbg !59
  %74 = load i32, ptr %v, align 4, !dbg !59
  %75 = load i32, ptr %v, align 4, !dbg !60
  %76 = icmp eq i32 %75, 0, !dbg !60
  br i1 %76, label %then28, label %else29, !dbg !60

then28:                                           ; preds = %if.end27
  store i32 1, ptr %err, align 4, !dbg !61
  %77 = load i32, ptr %err, align 4, !dbg !61
  br label %if.end30, !dbg !61

else29:                                           ; preds = %if.end27
  br label %if.end30, !dbg !61

if.end30:                                         ; preds = %else29, %then28
  %78 = load i32, ptr %a, align 4, !dbg !62
  %79 = load i32, ptr %c, align 4, !dbg !62
  %80 = icmp eq i32 %78, %79, !dbg !62
  %81 = zext i1 %80 to i32, !dbg !62
  store i32 %81, ptr %v, align 4, !dbg !62
  %82 = load i32, ptr %v, align 4, !dbg !62
  %83 = load i32, ptr %v, align 4, !dbg !63
  %84 = icmp ne i32 %83, 0, !dbg !63
  br i1 %84, label %then31, label %else32, !dbg !63

then31:                                           ; preds = %if.end30
  store i32 1, ptr %err, align 4, !dbg !64
  %85 = load i32, ptr %err, align 4, !dbg !64
  br label %if.end33, !dbg !64

else32:                                           ; preds = %if.end30
  br label %if.end33, !dbg !64

if.end33:                                         ; preds = %else32, %then31
  %86 = load i32, ptr %a, align 4, !dbg !65
  %87 = load i32, ptr %b, align 4, !dbg !65
  %88 = icmp ne i32 %86, %87, !dbg !65
  %89 = zext i1 %88 to i32, !dbg !65
  store i32 %89, ptr %v, align 4, !dbg !65
  %90 = load i32, ptr %v, align 4, !dbg !65
  %91 = load i32, ptr %v, align 4, !dbg !66
  %92 = icmp ne i32 %91, 0, !dbg !66
  br i1 %92, label %then34, label %else35, !dbg !66

then34:                                           ; preds = %if.end33
  store i32 1, ptr %err, align 4, !dbg !67
  %93 = load i32, ptr %err, align 4, !dbg !67
  br label %if.end36, !dbg !67

else35:                                           ; preds = %if.end33
  br label %if.end36, !dbg !67

if.end36:                                         ; preds = %else35, %then34
  %94 = load i32, ptr %a, align 4, !dbg !68
  %95 = load i32, ptr %c, align 4, !dbg !68
  %96 = icmp ne i32 %94, %95, !dbg !68
  %97 = zext i1 %96 to i32, !dbg !68
  store i32 %97, ptr %v, align 4, !dbg !68
  %98 = load i32, ptr %v, align 4, !dbg !68
  %99 = load i32, ptr %v, align 4, !dbg !69
  %100 = icmp eq i32 %99, 0, !dbg !69
  br i1 %100, label %then37, label %else38, !dbg !69

then37:                                           ; preds = %if.end36
  store i32 1, ptr %err, align 4, !dbg !70
  %101 = load i32, ptr %err, align 4, !dbg !70
  br label %if.end39, !dbg !70

else38:                                           ; preds = %if.end36
  br label %if.end39, !dbg !70

if.end39:                                         ; preds = %else38, %then37
  %102 = load i32, ptr %a, align 4, !dbg !71
  %103 = load i32, ptr %b, align 4, !dbg !71
  %104 = icmp slt i32 %102, %103, !dbg !71
  %105 = zext i1 %104 to i32, !dbg !71
  store i32 %105, ptr %v, align 4, !dbg !71
  %106 = load i32, ptr %v, align 4, !dbg !71
  %107 = load i32, ptr %v, align 4, !dbg !72
  %108 = icmp ne i32 %107, 0, !dbg !72
  br i1 %108, label %then40, label %else41, !dbg !72

then40:                                           ; preds = %if.end39
  store i32 1, ptr %err, align 4, !dbg !73
  %109 = load i32, ptr %err, align 4, !dbg !73
  br label %if.end42, !dbg !73

else41:                                           ; preds = %if.end39
  br label %if.end42, !dbg !73

if.end42:                                         ; preds = %else41, %then40
  %110 = load i32, ptr %a, align 4, !dbg !74
  %111 = load i32, ptr %c, align 4, !dbg !74
  %112 = icmp slt i32 %110, %111, !dbg !74
  %113 = zext i1 %112 to i32, !dbg !74
  store i32 %113, ptr %v, align 4, !dbg !74
  %114 = load i32, ptr %v, align 4, !dbg !74
  %115 = load i32, ptr %v, align 4, !dbg !75
  %116 = icmp eq i32 %115, 0, !dbg !75
  br i1 %116, label %then43, label %else44, !dbg !75

then43:                                           ; preds = %if.end42
  store i32 1, ptr %err, align 4, !dbg !76
  %117 = load i32, ptr %err, align 4, !dbg !76
  br label %if.end45, !dbg !76

else44:                                           ; preds = %if.end42
  br label %if.end45, !dbg !76

if.end45:                                         ; preds = %else44, %then43
  %118 = load i32, ptr %c, align 4, !dbg !77
  %119 = load i32, ptr %a, align 4, !dbg !77
  %120 = icmp slt i32 %118, %119, !dbg !77
  %121 = zext i1 %120 to i32, !dbg !77
  store i32 %121, ptr %v, align 4, !dbg !77
  %122 = load i32, ptr %v, align 4, !dbg !77
  %123 = load i32, ptr %v, align 4, !dbg !78
  %124 = icmp ne i32 %123, 0, !dbg !78
  br i1 %124, label %then46, label %else47, !dbg !78

then46:                                           ; preds = %if.end45
  store i32 1, ptr %err, align 4, !dbg !79
  %125 = load i32, ptr %err, align 4, !dbg !79
  br label %if.end48, !dbg !79

else47:                                           ; preds = %if.end45
  br label %if.end48, !dbg !79

if.end48:                                         ; preds = %else47, %then46
  %126 = load i32, ptr %a, align 4, !dbg !80
  %127 = load i32, ptr %b, align 4, !dbg !80
  %128 = icmp sle i32 %126, %127, !dbg !80
  %129 = zext i1 %128 to i32, !dbg !80
  store i32 %129, ptr %v, align 4, !dbg !80
  %130 = load i32, ptr %v, align 4, !dbg !80
  %131 = load i32, ptr %v, align 4, !dbg !81
  %132 = icmp eq i32 %131, 0, !dbg !81
  br i1 %132, label %then49, label %else50, !dbg !81

then49:                                           ; preds = %if.end48
  store i32 1, ptr %err, align 4, !dbg !82
  %133 = load i32, ptr %err, align 4, !dbg !82
  br label %if.end51, !dbg !82

else50:                                           ; preds = %if.end48
  br label %if.end51, !dbg !82

if.end51:                                         ; preds = %else50, %then49
  %134 = load i32, ptr %a, align 4, !dbg !83
  %135 = load i32, ptr %c, align 4, !dbg !83
  %136 = icmp sle i32 %134, %135, !dbg !83
  %137 = zext i1 %136 to i32, !dbg !83
  store i32 %137, ptr %v, align 4, !dbg !83
  %138 = load i32, ptr %v, align 4, !dbg !83
  %139 = load i32, ptr %v, align 4, !dbg !84
  %140 = icmp eq i32 %139, 0, !dbg !84
  br i1 %140, label %then52, label %else53, !dbg !84

then52:                                           ; preds = %if.end51
  store i32 1, ptr %err, align 4, !dbg !85
  %141 = load i32, ptr %err, align 4, !dbg !85
  br label %if.end54, !dbg !85

else53:                                           ; preds = %if.end51
  br label %if.end54, !dbg !85

if.end54:                                         ; preds = %else53, %then52
  %142 = load i32, ptr %c, align 4, !dbg !86
  %143 = load i32, ptr %a, align 4, !dbg !86
  %144 = icmp sle i32 %142, %143, !dbg !86
  %145 = zext i1 %144 to i32, !dbg !86
  store i32 %145, ptr %v, align 4, !dbg !86
  %146 = load i32, ptr %v, align 4, !dbg !86
  %147 = load i32, ptr %v, align 4, !dbg !87
  %148 = icmp ne i32 %147, 0, !dbg !87
  br i1 %148, label %then55, label %else56, !dbg !87

then55:                                           ; preds = %if.end54
  store i32 1, ptr %err, align 4, !dbg !88
  %149 = load i32, ptr %err, align 4, !dbg !88
  br label %if.end57, !dbg !88

else56:                                           ; preds = %if.end54
  br label %if.end57, !dbg !88

if.end57:                                         ; preds = %else56, %then55
  %150 = load i32, ptr %a, align 4, !dbg !89
  %151 = load i32, ptr %b, align 4, !dbg !89
  %152 = icmp sgt i32 %150, %151, !dbg !89
  %153 = zext i1 %152 to i32, !dbg !89
  store i32 %153, ptr %v, align 4, !dbg !89
  %154 = load i32, ptr %v, align 4, !dbg !89
  %155 = load i32, ptr %v, align 4, !dbg !90
  %156 = icmp ne i32 %155, 0, !dbg !90
  br i1 %156, label %then58, label %else59, !dbg !90

then58:                                           ; preds = %if.end57
  store i32 1, ptr %err, align 4, !dbg !91
  %157 = load i32, ptr %err, align 4, !dbg !91
  br label %if.end60, !dbg !91

else59:                                           ; preds = %if.end57
  br label %if.end60, !dbg !91

if.end60:                                         ; preds = %else59, %then58
  %158 = load i32, ptr %a, align 4, !dbg !92
  %159 = load i32, ptr %c, align 4, !dbg !92
  %160 = icmp sgt i32 %158, %159, !dbg !92
  %161 = zext i1 %160 to i32, !dbg !92
  store i32 %161, ptr %v, align 4, !dbg !92
  %162 = load i32, ptr %v, align 4, !dbg !92
  %163 = load i32, ptr %v, align 4, !dbg !93
  %164 = icmp ne i32 %163, 0, !dbg !93
  br i1 %164, label %then61, label %else62, !dbg !93

then61:                                           ; preds = %if.end60
  store i32 1, ptr %err, align 4, !dbg !94
  %165 = load i32, ptr %err, align 4, !dbg !94
  br label %if.end63, !dbg !94

else62:                                           ; preds = %if.end60
  br label %if.end63, !dbg !94

if.end63:                                         ; preds = %else62, %then61
  %166 = load i32, ptr %c, align 4, !dbg !95
  %167 = load i32, ptr %a, align 4, !dbg !95
  %168 = icmp sgt i32 %166, %167, !dbg !95
  %169 = zext i1 %168 to i32, !dbg !95
  store i32 %169, ptr %v, align 4, !dbg !95
  %170 = load i32, ptr %v, align 4, !dbg !95
  %171 = load i32, ptr %v, align 4, !dbg !96
  %172 = icmp eq i32 %171, 0, !dbg !96
  br i1 %172, label %then64, label %else65, !dbg !96

then64:                                           ; preds = %if.end63
  store i32 1, ptr %err, align 4, !dbg !97
  %173 = load i32, ptr %err, align 4, !dbg !97
  br label %if.end66, !dbg !97

else65:                                           ; preds = %if.end63
  br label %if.end66, !dbg !97

if.end66:                                         ; preds = %else65, %then64
  %174 = load i32, ptr %a, align 4, !dbg !98
  %175 = load i32, ptr %b, align 4, !dbg !98
  %176 = icmp sge i32 %174, %175, !dbg !98
  %177 = zext i1 %176 to i32, !dbg !98
  store i32 %177, ptr %v, align 4, !dbg !98
  %178 = load i32, ptr %v, align 4, !dbg !98
  %179 = load i32, ptr %v, align 4, !dbg !99
  %180 = icmp eq i32 %179, 0, !dbg !99
  br i1 %180, label %then67, label %else68, !dbg !99

then67:                                           ; preds = %if.end66
  store i32 1, ptr %err, align 4, !dbg !100
  %181 = load i32, ptr %err, align 4, !dbg !100
  br label %if.end69, !dbg !100

else68:                                           ; preds = %if.end66
  br label %if.end69, !dbg !100

if.end69:                                         ; preds = %else68, %then67
  %182 = load i32, ptr %a, align 4, !dbg !101
  %183 = load i32, ptr %c, align 4, !dbg !101
  %184 = icmp sge i32 %182, %183, !dbg !101
  %185 = zext i1 %184 to i32, !dbg !101
  store i32 %185, ptr %v, align 4, !dbg !101
  %186 = load i32, ptr %v, align 4, !dbg !101
  %187 = load i32, ptr %v, align 4, !dbg !102
  %188 = icmp ne i32 %187, 0, !dbg !102
  br i1 %188, label %then70, label %else71, !dbg !102

then70:                                           ; preds = %if.end69
  store i32 1, ptr %err, align 4, !dbg !103
  %189 = load i32, ptr %err, align 4, !dbg !103
  br label %if.end72, !dbg !103

else71:                                           ; preds = %if.end69
  br label %if.end72, !dbg !103

if.end72:                                         ; preds = %else71, %then70
  %190 = load i32, ptr %c, align 4, !dbg !104
  %191 = load i32, ptr %a, align 4, !dbg !104
  %192 = icmp sge i32 %190, %191, !dbg !104
  %193 = zext i1 %192 to i32, !dbg !104
  store i32 %193, ptr %v, align 4, !dbg !104
  %194 = load i32, ptr %v, align 4, !dbg !104
  %195 = load i32, ptr %v, align 4, !dbg !105
  %196 = icmp eq i32 %195, 0, !dbg !105
  br i1 %196, label %then73, label %else74, !dbg !105

then73:                                           ; preds = %if.end72
  store i32 1, ptr %err, align 4, !dbg !106
  %197 = load i32, ptr %err, align 4, !dbg !106
  br label %if.end75, !dbg !106

else74:                                           ; preds = %if.end72
  br label %if.end75, !dbg !106

if.end75:                                         ; preds = %else74, %then73
  store i32 1, ptr %ua, align 4, !dbg !15
  store i32 -1, ptr %ub, align 4, !dbg !13
  %198 = load i32, ptr %ua, align 4, !dbg !107
  %199 = load i32, ptr %ub, align 4, !dbg !107
  %200 = icmp ult i32 %198, %199, !dbg !107
  %201 = zext i1 %200 to i32, !dbg !107
  store i32 %201, ptr %v, align 4, !dbg !107
  %202 = load i32, ptr %v, align 4, !dbg !107
  %203 = load i32, ptr %v, align 4, !dbg !108
  %204 = icmp eq i32 %203, 0, !dbg !108
  br i1 %204, label %then76, label %else77, !dbg !108

then76:                                           ; preds = %if.end75
  store i32 1, ptr %err, align 4, !dbg !109
  %205 = load i32, ptr %err, align 4, !dbg !109
  br label %if.end78, !dbg !109

else77:                                           ; preds = %if.end75
  br label %if.end78, !dbg !109

if.end78:                                         ; preds = %else77, %then76
  %206 = load i32, ptr %ub, align 4, !dbg !110
  %207 = load i32, ptr %ua, align 4, !dbg !110
  %208 = icmp ugt i32 %206, %207, !dbg !110
  %209 = zext i1 %208 to i32, !dbg !110
  store i32 %209, ptr %v, align 4, !dbg !110
  %210 = load i32, ptr %v, align 4, !dbg !110
  %211 = load i32, ptr %v, align 4, !dbg !111
  %212 = icmp eq i32 %211, 0, !dbg !111
  br i1 %212, label %then79, label %else80, !dbg !111

then79:                                           ; preds = %if.end78
  store i32 1, ptr %err, align 4, !dbg !112
  %213 = load i32, ptr %err, align 4, !dbg !112
  br label %if.end81, !dbg !112

else80:                                           ; preds = %if.end78
  br label %if.end81, !dbg !112

if.end81:                                         ; preds = %else80, %then79
  store i32 -1, ptr %si, align 4, !dbg !11
  store i32 1, ptr %ui, align 4, !dbg !9
  %214 = load i32, ptr %si, align 4, !dbg !113
  %215 = load i32, ptr %ui, align 4, !dbg !113
  %216 = icmp ult i32 %214, %215, !dbg !113
  %217 = zext i1 %216 to i32, !dbg !113
  store i32 %217, ptr %v, align 4, !dbg !113
  %218 = load i32, ptr %v, align 4, !dbg !113
  %219 = load i32, ptr %v, align 4, !dbg !114
  %220 = icmp ne i32 %219, 0, !dbg !114
  br i1 %220, label %then82, label %else83, !dbg !114

then82:                                           ; preds = %if.end81
  store i32 1, ptr %err, align 4, !dbg !115
  %221 = load i32, ptr %err, align 4, !dbg !115
  br label %if.end84, !dbg !115

else83:                                           ; preds = %if.end81
  br label %if.end84, !dbg !115

if.end84:                                         ; preds = %else83, %then82
  %222 = load i32, ptr %si, align 4, !dbg !116
  %223 = load i32, ptr %ui, align 4, !dbg !116
  %224 = icmp ugt i32 %222, %223, !dbg !116
  %225 = zext i1 %224 to i32, !dbg !116
  store i32 %225, ptr %v, align 4, !dbg !116
  %226 = load i32, ptr %v, align 4, !dbg !116
  %227 = load i32, ptr %v, align 4, !dbg !117
  %228 = icmp eq i32 %227, 0, !dbg !117
  br i1 %228, label %then85, label %else86, !dbg !117

then85:                                           ; preds = %if.end84
  store i32 1, ptr %err, align 4, !dbg !118
  %229 = load i32, ptr %err, align 4, !dbg !118
  br label %if.end87, !dbg !118

else86:                                           ; preds = %if.end84
  br label %if.end87, !dbg !118

if.end87:                                         ; preds = %else86, %then85
  %230 = load i32, ptr %a, align 4, !dbg !119
  %231 = load i32, ptr %c, align 4, !dbg !119
  %232 = icmp slt i32 %230, %231, !dbg !119
  %233 = zext i1 %232 to i32, !dbg !119
  store i32 %233, ptr %v, align 4, !dbg !119
  %234 = load i32, ptr %v, align 4, !dbg !119
  %235 = load i32, ptr %v, align 4, !dbg !120
  %236 = icmp ne i32 %235, 1, !dbg !120
  br i1 %236, label %then88, label %else89, !dbg !120

then88:                                           ; preds = %if.end87
  store i32 1, ptr %err, align 4, !dbg !121
  %237 = load i32, ptr %err, align 4, !dbg !121
  br label %if.end90, !dbg !121

else89:                                           ; preds = %if.end87
  br label %if.end90, !dbg !121

if.end90:                                         ; preds = %else89, %then88
  %238 = load i32, ptr %a, align 4, !dbg !122
  %239 = load i32, ptr %b, align 4, !dbg !122
  %240 = icmp eq i32 %238, %239, !dbg !122
  %241 = zext i1 %240 to i32, !dbg !122
  store i32 %241, ptr %v, align 4, !dbg !122
  %242 = load i32, ptr %v, align 4, !dbg !122
  %243 = load i32, ptr %v, align 4, !dbg !123
  %244 = icmp ne i32 %243, 1, !dbg !123
  br i1 %244, label %then91, label %else92, !dbg !123

then91:                                           ; preds = %if.end90
  store i32 1, ptr %err, align 4, !dbg !124
  %245 = load i32, ptr %err, align 4, !dbg !124
  br label %if.end93, !dbg !124

else92:                                           ; preds = %if.end90
  br label %if.end93, !dbg !124

if.end93:                                         ; preds = %else92, %then91
  %246 = load i1, ptr %t, align 1, !dbg !125
  %247 = load i1, ptr %t, align 1, !dbg !125
  %248 = select i1 %246, i1 %247, i1 false, !dbg !125
  %249 = zext i1 %248 to i32, !dbg !125
  store i32 %249, ptr %v, align 4, !dbg !125
  %250 = load i32, ptr %v, align 4, !dbg !125
  %251 = load i32, ptr %v, align 4, !dbg !126
  %252 = icmp ne i32 %251, 1, !dbg !126
  br i1 %252, label %then94, label %else95, !dbg !126

then94:                                           ; preds = %if.end93
  store i32 1, ptr %err, align 4, !dbg !127
  %253 = load i32, ptr %err, align 4, !dbg !127
  br label %if.end96, !dbg !127

else95:                                           ; preds = %if.end93
  br label %if.end96, !dbg !127

if.end96:                                         ; preds = %else95, %then94
  %254 = load i1, ptr %t, align 1, !dbg !128
  %255 = load i1, ptr %f, align 1, !dbg !128
  %256 = select i1 %254, i1 true, i1 %255, !dbg !128
  %257 = zext i1 %256 to i32, !dbg !128
  store i32 %257, ptr %v, align 4, !dbg !128
  %258 = load i32, ptr %v, align 4, !dbg !128
  %259 = load i32, ptr %v, align 4, !dbg !129
  %260 = icmp ne i32 %259, 1, !dbg !129
  br i1 %260, label %then97, label %else98, !dbg !129

then97:                                           ; preds = %if.end96
  store i32 1, ptr %err, align 4, !dbg !130
  %261 = load i32, ptr %err, align 4, !dbg !130
  br label %if.end99, !dbg !130

else98:                                           ; preds = %if.end96
  br label %if.end99, !dbg !130

if.end99:                                         ; preds = %else98, %then97
  %262 = load i1, ptr %f, align 1, !dbg !131
  %263 = icmp eq i1 %262, false, !dbg !131
  %264 = zext i1 %263 to i32, !dbg !131
  store i32 %264, ptr %v, align 4, !dbg !131
  %265 = load i32, ptr %v, align 4, !dbg !131
  %266 = load i32, ptr %v, align 4, !dbg !132
  %267 = icmp ne i32 %266, 1, !dbg !132
  br i1 %267, label %then100, label %else101, !dbg !132

then100:                                          ; preds = %if.end99
  store i32 1, ptr %err, align 4, !dbg !133
  %268 = load i32, ptr %err, align 4, !dbg !133
  br label %if.end102, !dbg !133

else101:                                          ; preds = %if.end99
  br label %if.end102, !dbg !133

if.end102:                                        ; preds = %else101, %then100
  %269 = load i32, ptr %a, align 4, !dbg !134
  %270 = load i32, ptr %c, align 4, !dbg !134
  %271 = icmp slt i32 %269, %270, !dbg !134
  %272 = zext i1 %271 to i32, !dbg !134
  %273 = mul i32 %272, 3, !dbg !134
  %274 = icmp ne i32 %273, 3, !dbg !134
  br i1 %274, label %then103, label %else104, !dbg !134

then103:                                          ; preds = %if.end102
  store i32 1, ptr %err, align 4, !dbg !135
  %275 = load i32, ptr %err, align 4, !dbg !135
  br label %if.end105, !dbg !135

else104:                                          ; preds = %if.end102
  br label %if.end105, !dbg !135

if.end105:                                        ; preds = %else104, %then103
  %276 = load i32, ptr %a, align 4, !dbg !136
  %277 = load i32, ptr %c, align 4, !dbg !136
  %278 = icmp slt i32 %276, %277, !dbg !136
  %279 = zext i1 %278 to i32, !dbg !136
  %280 = add i32 %279, 10, !dbg !136
  %281 = icmp ne i32 %280, 11, !dbg !136
  br i1 %281, label %then106, label %else107, !dbg !136

then106:                                          ; preds = %if.end105
  store i32 1, ptr %err, align 4, !dbg !137
  %282 = load i32, ptr %err, align 4, !dbg !137
  br label %if.end108, !dbg !137

else107:                                          ; preds = %if.end105
  br label %if.end108, !dbg !137

if.end108:                                        ; preds = %else107, %then106
  %283 = load i32, ptr %a, align 4, !dbg !138
  %284 = load i32, ptr %c, align 4, !dbg !138
  %285 = icmp slt i32 %283, %284, !dbg !138
  %286 = zext i1 %285 to i32, !dbg !138
  %287 = sub i32 0, %286, !dbg !138
  %288 = icmp ne i32 %287, -1, !dbg !138
  br i1 %288, label %then109, label %else110, !dbg !138

then109:                                          ; preds = %if.end108
  store i32 1, ptr %err, align 4, !dbg !139
  %289 = load i32, ptr %err, align 4, !dbg !139
  br label %if.end111, !dbg !139

else110:                                          ; preds = %if.end108
  br label %if.end111, !dbg !139

if.end111:                                        ; preds = %else110, %then109
  %290 = load i32, ptr %a, align 4, !dbg !140
  %291 = load i32, ptr %c, align 4, !dbg !140
  %292 = icmp slt i32 %290, %291, !dbg !140
  %293 = zext i1 %292 to i32, !dbg !140
  %294 = icmp slt i32 %293, 5, !dbg !140
  %295 = zext i1 %294 to i32, !dbg !140
  %296 = icmp ne i32 %295, 1, !dbg !140
  br i1 %296, label %then112, label %else113, !dbg !140

then112:                                          ; preds = %if.end111
  store i32 1, ptr %err, align 4, !dbg !141
  %297 = load i32, ptr %err, align 4, !dbg !141
  br label %if.end114, !dbg !141

else113:                                          ; preds = %if.end111
  br label %if.end114, !dbg !141

if.end114:                                        ; preds = %else113, %then112
  %298 = load i32, ptr %err, align 4, !dbg !142
  %299 = icmp eq i32 %298, 0, !dbg !142
  br i1 %299, label %then115, label %else116, !dbg !142

then115:                                          ; preds = %if.end114
  %300 = call i32 (ptr, ...) @printf(ptr @0), !dbg !143
  br label %if.end117, !dbg !143

else116:                                          ; preds = %if.end114
  %301 = call i32 (ptr, ...) @printf(ptr @1), !dbg !145
  br label %if.end117, !dbg !145

if.end117:                                        ; preds = %else116, %then115
  %302 = load i32, ptr %err, align 4, !dbg !147
  ret i32 %302, !dbg !147
}

!llvm.dbg.cu = !{!0}

!0 = distinct !DICompileUnit(language: DW_LANG_C, file: !1, producer: "lcc", isOptimized: false, runtimeVersion: 0, emissionKind: FullDebug)
!1 = !DIFile(filename: "15.logic.c", directory: "../tests")
!2 = distinct !DISubprogram(name: "main", linkageName: "main", scope: null, file: !1, line: 6, type: !3, scopeLine: 6, spFlags: DISPFlagDefinition, unit: !0, retainedNodes: !6)
!3 = !DISubroutineType(types: !4)
!4 = !{!5}
!5 = !DIBasicType(name: "int", size: 32, encoding: DW_ATE_signed)
!6 = !{}
!7 = !DILocalVariable(name: "ui", scope: !2, file: !1, line: 79, type: !8)
!8 = !DIBasicType(name: "unsigned int", size: 32, encoding: DW_ATE_unsigned)
!9 = !DILocation(line: 79, column: 3, scope: !2)
!10 = !DILocalVariable(name: "si", scope: !2, file: !1, line: 78, type: !5)
!11 = !DILocation(line: 78, column: 3, scope: !2)
!12 = !DILocalVariable(name: "ub", scope: !2, file: !1, line: 72, type: !8)
!13 = !DILocation(line: 72, column: 3, scope: !2)
!14 = !DILocalVariable(name: "ua", scope: !2, file: !1, line: 71, type: !8)
!15 = !DILocation(line: 71, column: 3, scope: !2)
!16 = !DILocalVariable(name: "c", scope: !2, file: !1, line: 36, type: !5)
!17 = !DILocation(line: 36, column: 3, scope: !2)
!18 = !DILocalVariable(name: "b", scope: !2, file: !1, line: 35, type: !5)
!19 = !DILocation(line: 35, column: 3, scope: !2)
!20 = !DILocalVariable(name: "a", scope: !2, file: !1, line: 34, type: !5)
!21 = !DILocation(line: 34, column: 3, scope: !2)
!22 = !DILocalVariable(name: "v", scope: !2, file: !1, line: 10, type: !5)
!23 = !DILocation(line: 10, column: 3, scope: !2)
!24 = !DILocalVariable(name: "f", scope: !2, file: !1, line: 9, type: !25)
!25 = !DIBasicType(name: "bool", size: 8, encoding: DW_ATE_boolean)
!26 = !DILocation(line: 9, column: 3, scope: !2)
!27 = !DILocalVariable(name: "t", scope: !2, file: !1, line: 8, type: !25)
!28 = !DILocation(line: 8, column: 3, scope: !2)
!29 = !DILocalVariable(name: "err", scope: !2, file: !1, line: 7, type: !5)
!30 = !DILocation(line: 7, column: 3, scope: !2)
!31 = !DILocation(line: 12, column: 3, scope: !2)
!32 = !DILocation(line: 12, column: 15, scope: !2)
!33 = !DILocation(line: 13, column: 3, scope: !2)
!34 = !DILocation(line: 13, column: 15, scope: !2)
!35 = !DILocation(line: 15, column: 3, scope: !2)
!36 = !DILocation(line: 16, column: 3, scope: !2)
!37 = !DILocation(line: 16, column: 15, scope: !2)
!38 = !DILocation(line: 17, column: 3, scope: !2)
!39 = !DILocation(line: 18, column: 3, scope: !2)
!40 = !DILocation(line: 18, column: 15, scope: !2)
!41 = !DILocation(line: 19, column: 3, scope: !2)
!42 = !DILocation(line: 20, column: 3, scope: !2)
!43 = !DILocation(line: 20, column: 15, scope: !2)
!44 = !DILocation(line: 22, column: 3, scope: !2)
!45 = !DILocation(line: 23, column: 3, scope: !2)
!46 = !DILocation(line: 23, column: 15, scope: !2)
!47 = !DILocation(line: 24, column: 3, scope: !2)
!48 = !DILocation(line: 25, column: 3, scope: !2)
!49 = !DILocation(line: 25, column: 15, scope: !2)
!50 = !DILocation(line: 26, column: 3, scope: !2)
!51 = !DILocation(line: 27, column: 3, scope: !2)
!52 = !DILocation(line: 27, column: 15, scope: !2)
!53 = !DILocation(line: 29, column: 3, scope: !2)
!54 = !DILocation(line: 30, column: 3, scope: !2)
!55 = !DILocation(line: 30, column: 15, scope: !2)
!56 = !DILocation(line: 31, column: 3, scope: !2)
!57 = !DILocation(line: 32, column: 3, scope: !2)
!58 = !DILocation(line: 32, column: 15, scope: !2)
!59 = !DILocation(line: 38, column: 3, scope: !2)
!60 = !DILocation(line: 39, column: 3, scope: !2)
!61 = !DILocation(line: 39, column: 15, scope: !2)
!62 = !DILocation(line: 40, column: 3, scope: !2)
!63 = !DILocation(line: 41, column: 3, scope: !2)
!64 = !DILocation(line: 41, column: 15, scope: !2)
!65 = !DILocation(line: 42, column: 3, scope: !2)
!66 = !DILocation(line: 43, column: 3, scope: !2)
!67 = !DILocation(line: 43, column: 15, scope: !2)
!68 = !DILocation(line: 44, column: 3, scope: !2)
!69 = !DILocation(line: 45, column: 3, scope: !2)
!70 = !DILocation(line: 45, column: 15, scope: !2)
!71 = !DILocation(line: 46, column: 3, scope: !2)
!72 = !DILocation(line: 47, column: 3, scope: !2)
!73 = !DILocation(line: 47, column: 15, scope: !2)
!74 = !DILocation(line: 48, column: 3, scope: !2)
!75 = !DILocation(line: 49, column: 3, scope: !2)
!76 = !DILocation(line: 49, column: 15, scope: !2)
!77 = !DILocation(line: 50, column: 3, scope: !2)
!78 = !DILocation(line: 51, column: 3, scope: !2)
!79 = !DILocation(line: 51, column: 15, scope: !2)
!80 = !DILocation(line: 52, column: 3, scope: !2)
!81 = !DILocation(line: 53, column: 3, scope: !2)
!82 = !DILocation(line: 53, column: 15, scope: !2)
!83 = !DILocation(line: 54, column: 3, scope: !2)
!84 = !DILocation(line: 55, column: 3, scope: !2)
!85 = !DILocation(line: 55, column: 15, scope: !2)
!86 = !DILocation(line: 56, column: 3, scope: !2)
!87 = !DILocation(line: 57, column: 3, scope: !2)
!88 = !DILocation(line: 57, column: 15, scope: !2)
!89 = !DILocation(line: 58, column: 3, scope: !2)
!90 = !DILocation(line: 59, column: 3, scope: !2)
!91 = !DILocation(line: 59, column: 15, scope: !2)
!92 = !DILocation(line: 60, column: 3, scope: !2)
!93 = !DILocation(line: 61, column: 3, scope: !2)
!94 = !DILocation(line: 61, column: 15, scope: !2)
!95 = !DILocation(line: 62, column: 3, scope: !2)
!96 = !DILocation(line: 63, column: 3, scope: !2)
!97 = !DILocation(line: 63, column: 15, scope: !2)
!98 = !DILocation(line: 64, column: 3, scope: !2)
!99 = !DILocation(line: 65, column: 3, scope: !2)
!100 = !DILocation(line: 65, column: 15, scope: !2)
!101 = !DILocation(line: 66, column: 3, scope: !2)
!102 = !DILocation(line: 67, column: 3, scope: !2)
!103 = !DILocation(line: 67, column: 15, scope: !2)
!104 = !DILocation(line: 68, column: 3, scope: !2)
!105 = !DILocation(line: 69, column: 3, scope: !2)
!106 = !DILocation(line: 69, column: 15, scope: !2)
!107 = !DILocation(line: 73, column: 3, scope: !2)
!108 = !DILocation(line: 74, column: 3, scope: !2)
!109 = !DILocation(line: 74, column: 15, scope: !2)
!110 = !DILocation(line: 75, column: 3, scope: !2)
!111 = !DILocation(line: 76, column: 3, scope: !2)
!112 = !DILocation(line: 76, column: 15, scope: !2)
!113 = !DILocation(line: 80, column: 3, scope: !2)
!114 = !DILocation(line: 81, column: 3, scope: !2)
!115 = !DILocation(line: 81, column: 15, scope: !2)
!116 = !DILocation(line: 82, column: 3, scope: !2)
!117 = !DILocation(line: 83, column: 3, scope: !2)
!118 = !DILocation(line: 83, column: 15, scope: !2)
!119 = !DILocation(line: 88, column: 3, scope: !2)
!120 = !DILocation(line: 89, column: 3, scope: !2)
!121 = !DILocation(line: 89, column: 15, scope: !2)
!122 = !DILocation(line: 90, column: 3, scope: !2)
!123 = !DILocation(line: 91, column: 3, scope: !2)
!124 = !DILocation(line: 91, column: 15, scope: !2)
!125 = !DILocation(line: 92, column: 3, scope: !2)
!126 = !DILocation(line: 93, column: 3, scope: !2)
!127 = !DILocation(line: 93, column: 15, scope: !2)
!128 = !DILocation(line: 94, column: 3, scope: !2)
!129 = !DILocation(line: 95, column: 3, scope: !2)
!130 = !DILocation(line: 95, column: 15, scope: !2)
!131 = !DILocation(line: 96, column: 3, scope: !2)
!132 = !DILocation(line: 97, column: 3, scope: !2)
!133 = !DILocation(line: 97, column: 15, scope: !2)
!134 = !DILocation(line: 101, column: 3, scope: !2)
!135 = !DILocation(line: 101, column: 25, scope: !2)
!136 = !DILocation(line: 102, column: 3, scope: !2)
!137 = !DILocation(line: 102, column: 27, scope: !2)
!138 = !DILocation(line: 103, column: 3, scope: !2)
!139 = !DILocation(line: 103, column: 23, scope: !2)
!140 = !DILocation(line: 104, column: 3, scope: !2)
!141 = !DILocation(line: 104, column: 27, scope: !2)
!142 = !DILocation(line: 106, column: 3, scope: !2)
!143 = !DILocation(line: 107, column: 5, scope: !144)
!144 = distinct !DILexicalBlock(scope: !2, file: !1, line: 106, column: 17)
!145 = !DILocation(line: 109, column: 5, scope: !146)
!146 = distinct !DILexicalBlock(scope: !2, file: !1, line: 108, column: 10)
!147 = !DILocation(line: 111, column: 3, scope: !2)
