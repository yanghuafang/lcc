...
`lcc` targets **macOS** (Homebrew) and **Ubuntu**. You need LLVM 20, flex, bison, argparse, graphviz, doxygen, CMake 3.22+, a C++17 compiler, and a system linker (`clang` or `gcc`).
...
- Three scripts check the tree, and CI runs all of them — run them before committing:
...
- Write commit messages as **Conventional Commits** — `type(scope): description`, lowercase, imperative, no trailing period — matching the existing history, e.g. `fix(codegen): zero-initialize bare globals`. Use the type that describes the change: `feat`, `fix`, `refactor`, `docs`, `build` or `test`. Milestones live in the docs; do not tag them in a commit message.
...
- Describe **what** changed and **why** in the PR, and confirm CI passes on Ubuntu and macOS.
