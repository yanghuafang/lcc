...
Smoke mode only checks that every variant **builds and runs correctly**; it does not assert performance thresholds. CI runs `./bench.sh --smoke` in [`.github/workflows/build.yml`](../.github/workflows/build.yml).
...
### 2025-06-29 — macOS arm64 (Apple M2 Pro), LLVM 20
