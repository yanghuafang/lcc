...
with `new` in `Parser.y`, and `ast/Ownership.cpp` frees them with `delete`.
Modern C++ would reach for `std::unique_ptr`, and this is the one place lcc
does not. The reason is bison, not preference.

lcc uses bison's **C skeleton**: `yyparse()` keeps semantic values in a stack of
`YYSTYPE`, a `union` of member types, and copies them by assignment. A
...
"lalr1.cc"` with `%define api.value.type variant` — which rewrites every line
of the grammar, the flex interface it shares, `main()`'s use of `yyparse()` and
`g_root`, and every `Node*` member the walkers under `irgen/` read. That
...
| Leak when a parse fails partway | `%destructor` on the declaration groups in `Parser.y`, which bison runs on the symbols still on its stack. **Not complete:** the nodes a partial `AST::Decls` holds still leak. The gap is deliberate and recorded in `.github/workflows/sanitizer.yml`; no test in the suite fails to parse, so CI never reaches it, and adding one means fixing the leak first |
| Double free through a copied node | `Node`'s copy constructor and copy assignment are deleted, so every node class is non-copyable and a shallow copy is a compile error |
...
**Three kinds of `new` live in `src/`, and only one is the deviation.** The ones
...
| `types/TypeRules.hpp` | `namespace typerules` — C type rules LLVM cannot express: signedness predicates, integer promotion, usual arithmetic conversion. A leaf: pure functions over `BuiltinTypeId`, with no LLVM or AST dependency. They are `constexpr` and defined here rather than in a `.cpp`, because a rule that reads only its arguments can be evaluated while lcc is compiled |
| `types/TypeRules.cpp` | `static_assert`s over the header's rules — the promotion table and the conversion ladder, checked at build time. Holds no rules of its own |
