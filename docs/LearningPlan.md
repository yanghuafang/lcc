...
**Key idea:** IR **generation** is `genCode()` + `IRBuilder`; LLVM only ever sees IR that lcc already built. `IrOptimizer` runs the custom passes first, then the LLVM pipeline. `-l-pre-opt` / `-l-post-opt` bracket the middle-end; `-l` dumps after `genObjectCode()`, with target metadata attached.
...
**Goal:** Confirm the build and the 41-test regression suite.
...
| Verify | Can explain the path `yyparse` → `g_root` → `genIrCode` → `genObjectCode` |

**Deep-dive files:** `AbstractSyntaxTree.hpp` (header comment), `VarDecl::genCode`, `FuncDecl::genCode`.
...
| Study | Opaque pointers: pointee types live on `VarType`, not on `llvm::Type*` |
...
| Step | Action |
|------|--------|
| Implement | `src/IrOptimizer.hpp` / `.cpp`; move `optimizeCode()` out of `CodeGenerator` |
...
| Implement | `IrInstructionStatsPass` in `src/passes/`; enable with `-ir-stats <file>` (`-` = stderr) |
...
| Step | Action |
|------|--------|
| Implement | `FoldAddZeroPass` in `src/passes/` — rewrites `add iN %x, 0` to `%x`; enable with `-fold-add-zero` |
...
| Implement | Write the notes up in [LlvmTools.md](LlvmTools.md); leave a pointer comment in `IrOptimizer.cpp` |
...
| Step | Action |
|------|--------|
| Implement | `src/TargetBackend.hpp` / `.cpp`; move `genObjectCode()` out of `CodeGenerator` |
...
| Step | Action |
|------|--------|
| Study | Compile `tests/40.array_sum.c` at `-O3` (a study fixture, deliberately outside the 41-test suite) |
...
| AST ownership | `AbstractSyntaxTree.hpp` header comment |
| Single-pass typing | `getExprTypeId`, `getExprVarType` during `genCode()` |
| Debug info | `DebugInfoBuilder.cpp`, the `-g` path |
...
| `AbstractSyntaxTree.hpp` comment block | lcc's single-pass architecture |
