...
`llvm-20` and `libargparse-dev` are available from Ubuntu apt on current LTS releases.
...
Building `lcc` (or running `bison` on `Parser.y`) reports grammar conflicts. That is expected for this compact grammar: Bison resolves them with default rules, and the unit tests still pass. For a learner-oriented breakdown, see [ParserConflicts.md](ParserConflicts.md).
