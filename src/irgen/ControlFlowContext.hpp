...
/// Like SymbolTable, this emits no IR — it only records which llvm::BasicBlock
/// a jump should land in — which is what makes it readable, and testable, on
/// its own.
...
/// RAII guards for the continue, break and fall-through stacks, and the way
/// StmtToIr.cpp is meant to reach them: nothing calls enterLoop / leaveLoop or
/// enterSwitch / leaveSwitch by hand. They live here rather than at the call
/// sites for the reason ScopedSymbolTable lives beside its stack in
/// irgen/SymbolTable.hpp — the pairing is visible next to what it balances, and
/// a body that throws partway through cannot leave a target pushed behind it.
///
...
/// is the whole reason `continue` inside a switch reaches the enclosing loop,
/// so it is worth two types that cannot be confused for each other.
class ScopedLoop {
 public:
