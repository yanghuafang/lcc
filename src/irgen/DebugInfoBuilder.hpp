...
//   2. setTypeEnv(env)          struct/union DI has to read AST field lists and
...
// entry-block allocas. Once mem2reg promotes those allocas to SSA registers the
// declares are stale, and recovering locations then needs dbg.value salvaging,
// which lcc does not implement. That is the whole reason -g blanks -O.
//
...
class DebugInfoBuilder {
 public:
  ...
  // Step 1: create the DWARF compile unit. sourcePath is the user's -i path, so
  // that a debugger resolves the original .c file rather than anything lcc
