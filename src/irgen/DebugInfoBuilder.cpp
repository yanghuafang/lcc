...
void DebugInfoBuilder::setTypeEnv(TypeEnv* env) { typeEnv_ = env; }

llvm::DIType* DebugInfoBuilder::getOrCreateVoidType() {
  return dib_->createBasicType("void", 0, llvm::dwarf::DW_ATE_address);
}

llvm::DIType* DebugInfoBuilder::getOrCreateBuiltinDIType(BuiltinTypeId typeId) {
  switch (typeId) {
...
llvm::DIType* DebugInfoBuilder::getOrCreateStructDIType(
  ...
  if (structType == nullptr || typeEnv_ == nullptr) {
  ...
  llvm::Type* llvmType = structType->getType(*typeEnv_);
  auto* llvmStruct = llvm::cast<llvm::StructType>(llvmType);
  ...
  for (AST::FieldDecl* field : *structType->structBody_) {
    llvm::Type* fieldLlvmType = field->varType_->getType(*typeEnv_);
...
llvm::DIType* DebugInfoBuilder::getOrCreateUnionDIType(
  ...
  if (unionType == nullptr || typeEnv_ == nullptr) {
  ...
  llvm::Type* llvmType = unionType->getType(*typeEnv_);
  const uint64_t sizeInBits =
      module_.getDataLayout().getTypeSizeInBits(llvmType);
  const unsigned alignInBits =
      module_.getDataLayout().getABITypeAlign(llvmType).value() * 8;
...
llvm::DIType* DebugInfoBuilder::getOrCreateDIType(llvm::Type* llvmType,
                                                  AST::VarType* varType) {
  // LLVM integer types carry no signedness; prefer the AST VarType when
  // present.
  ...
  // Opaque LLVM pointer types carry no pointee; build pointer DI from AST
  // baseType_.
  if (varType != nullptr && varType->isPointerType()) {
    AST::VarType* pointee = static_cast<AST::PointerType*>(varType)->baseType_;
  ...
  if (varType != nullptr && varType->isArrayType()) {
    ...
    return dib_->createArrayType(
        module_.getDataLayout().getTypeAllocSizeInBits(
            varType->getType(*typeEnv_)),
        module_.getDataLayout()
                .getABITypeAlign(varType->getType(*typeEnv_))
                .value() *
            8,
        elementDi, subscripts);
  }
  ...
  if (varType != nullptr && varType->isEnumType()) {
    return getOrCreateBuiltinDIType(BuiltinTypeId::INT);
  }
...
llvm::DIType* DebugInfoBuilder::getOrCreateLlvmType(llvm::Type* type) {
  ...
  // No AST VarType (e.g. extern void*): opaque ptr in IR maps to void* in
  // DWARF.
  if (type->isPointerTy()) {
    const unsigned pointerBits =
        module_.getDataLayout().getPointerSizeInBits(0);
  ...
  if (type->isStructTy()) {
    ...
    if (typeEnv_ != nullptr) {
      if (AST::StructType* astStruct = typeEnv_->findStructType(structTy)) {
        return getOrCreateStructDIType(astStruct);
      }
      if (AST::UnionType* astUnion = typeEnv_->findUnionType(structTy)) {
...
llvm::DISubprogram* DebugInfoBuilder::createFunction(
  ...
  llvm::DISubroutineType* signature =
      dib_->createSubroutineType(dib_->getOrCreateTypeArray(paramTypes));
...
void DebugInfoBuilder::insertAllocaDeclare(llvm::AllocaInst* alloca,
  ...
  // insertDeclare expects an iterator insert position (Instruction* overload
  // removed).
...
void DebugInfoBuilder::declareAlloca(llvm::AllocaInst* alloca,
                                     llvm::DIScope* scope,
                                     const std::string& name,
                                     llvm::Type* llvmType,
                                     AST::VarType* varType, unsigned line,
                                     unsigned col, unsigned paramArgNo) {
  ...
  if (paramArgNo > 0) {
    variable = dib_->createParameterVariable(scope, name, paramArgNo, file_,
                                             line, diType);
  } else {
