...
class SymbolTable {
 ...
 private:
  ...
  /// One scope level. The stacks below own these by value: a scope is pushed
  /// and popped, never shared, so there is nothing for a pointer to express
  /// here except a chance to leak one. Unwinding out of a half-finished walk
  /// drains them without help from a destructor.
