...
// global instead, named <function>.<var> so two functions may each declare `n`,
// with InternalLinkage so the name never escapes the translation unit.
//
...
// initializer is a constant that costs nothing — the global carries it. When it
// is not, the lowering has to emit a guard:
//
//     if (!<var>.inited) { <initializer>; <var>.inited = true; }
//
// which means splitting the current basic block in two mid-declaration. That is
