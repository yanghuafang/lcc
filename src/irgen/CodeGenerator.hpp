...
///
/// findType, findTypedefAlias, addConstant and the struct and union registry
/// pairs forward straight into symbols(). They exist as members because TypeEnv
/// is the interface types/ and the irgen services are written against — they
/// must not have to know a SymbolTable exists — not because a facade was wanted
/// for its own sake. Nothing else forwards.
///
