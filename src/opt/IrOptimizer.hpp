...
struct IrOptimizerOptions {
  ...
  /// Run FoldAddZeroPass before LLVM opts (teaching transform).
  bool foldAddZero = false;
  /// When non-empty, run this New PM pipeline instead of default<O*>.
...
/// no -ir-stats, no -fold-add-zero means the module comes back untouched. So
/// the driver never has to decide whether optimization applies.
///
...
///      describe what the front end actually produced, before LLVM
///      canonicalizes it out of recognition.
