...
/// here, so adding a scalar type such as `long long` is one edit rather than
/// two switches that can silently drift apart.
///
...
/// no AST::VarType. Its callers needed nothing else from that header, so
/// reaching this table meant including every VarType query to call the one
/// function that queries no VarType at all.
