...
bool MachineInstrStatsPass::runOnMachineFunction(llvm::MachineFunction& function) {
  // Unlike the New PM passes here, no isDeclaration() guard is needed: the legacy
  // pass manager only runs machine passes over defined functions, which have MIR.
  uint64_t count = 0;
...
bool MachineInstrStatsPass::doFinalization(llvm::Module&) {
