...
int main(int argc, char* argv[]) {
  ...
  parser.add_argument("-fold-add-zero", "--fold-add-zero")
      .default_value(false)
      .implicit_value(true)
      .help("fold add iN %x, 0 to %x before LLVM opts (teaching pass).");
  ...
  parser.add_argument("-machine-stats", "--machine-instruction-stats-file")
      ...
      .help(
          ...
          "stderr); legacy MachineFunctionPass. Omit to disable.");
  ...
  const std::string customPipeline = parser.get<std::string>("-O-passes");
