...
//
// The largest section, because C's expression grammar is the
...
/// Base of every C expression, and of every class below it. Derives from
/// Stmt because C's expression-statement is just `expression ;` -- the
/// semicolon carries no semantics, so an expression can stand wherever a
/// statement can, and needs no wrapper node. clang's AST makes the same
/// choice.
///
