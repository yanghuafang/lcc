...
// like it hands the job to Bison, but a %destructor runs only for a *discarded*
// symbol — one popped during error recovery, or left on the stack when the
// parse aborts. Nothing destroys a symbol that a successful reduction consumed:
// in the generated parser, yydestruct is reachable only from the error and
// cleanup labels, never from yyreduce. A clean parse therefore frees nothing,
// and each identifier occurrence leaks its std::string.
//
// Per-action `delete $N` would fix it, but the grammar mentions IDENTIFIER in
// around thirty places, and every rule added later would have to remember. So
// the front end owns the strings instead: the lexer allocates through create(),
// the grammar actions copy the text into AST nodes (which hold std::string by
// value), and the driver calls releaseAll() once the parse is over. No rule
// owns anything, so no rule can leak.
