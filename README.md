...
| **A real toolchain** | Emits genuine **LLVM 14** IR and native object files through `IRBuilder` and `llvm::TargetMachine` — not a toy backend. |
...
There is **no separate semantic-analysis pass**: C type information is resolved on demand during `genCode()` as it emits IR. Link the resulting `.o` with `clang` or `gcc`.
...
Build artifacts go to `../../lcc-build/` (sibling of the repo). Sample AST graphs and LLVM IR live under `lcc/debug/` (IR files use `.ll`, `.debug.ll`, or `.release.ll` suffixes matching [compile modes](docs/Testing.md)).
...
`lcc` emits non-PIC objects, so on Linux the link step is `clang /tmp/sum.o -o /tmp/sum -no-pie`. Add `-O2` to optimize, `-g` for a debuggable build, or `-v /tmp/sum.dot` to dump the AST graph. Full flag reference: [docs/Usage.md](docs/Usage.md).
