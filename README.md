...
| `-l` | no | LLVM IR (`.ll`) |
| `-g` | no | Embed DWARF in the object file (6a: compile unit + function subprograms; use with `-O0` for reliable stepping) |
...
- Finish `-g` debug info: line stepping (6b), local variables (6c), struct scopes (6d). Function subprograms and DWARF compile unit are done (6a).