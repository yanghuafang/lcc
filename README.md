...
`llvm-14` and `libargparse-dev` are available from Ubuntu apt on current LTS releases.
...
../../lcc-build/lcc -i ../tests/0.hello_world.c -o ../../lcc-build/0.hello_world.o \
  -v ../debug/0.hello_world.dot -l ../debug/0.hello_world.release.ll
```
...
| `install-deps-ubuntu.sh` | Install apt packages on Ubuntu LTS (LLVM 14, flex, bison, etc.) |
...
For step-by-step detail (dependencies, tests, and legal/illegal forms), see [`docs/FrontendNotes.md`](docs/FrontendNotes.md). Array work through 2D and typedef (builtin and struct/union) are done; 3D arrays are deferred.
